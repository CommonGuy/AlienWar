...
public class CartographerLongVisionAlien extends Alien
{
  ...
  // Map constants
  private static final int MAP_SIZE_DIV_2 = 10;
  private static final int MAP_SIZE = MAP_SIZE_DIV_2 * 2 + 1;
  ...
  private static final int SIMULATED_BATTLE_THREASHOLD = SIMULATED_BATTLE_COUNT;

  private Random rand = new Random();

  /** The alien's map of the immediate and mid-range area */
  private double[][] map = new double[MAP_SIZE][MAP_SIZE];
...
//    }
  ...
  /**
   * Movement Priority: 1. move to collide with predictable prey if any seen. 2. if in beginning of match, avoid all Aliens. 3. if
   * not in beginning of match: 3a. coalesce with other aliens if any aliens present. 3b. move in a predictable manner to maintain
   * coalescence.
   */
  ...
  {
    double toughnessFactor =((double) enemyAbilities[STR])/(enemyAbilities[LIFE]*4); // a fudge-factor to ensure that whales are attacked.
